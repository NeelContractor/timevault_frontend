...
export function useTimevaultProgram() {
  ...
  const accounts = useQuery({
    ...
    queryFn: async () => {
      if (!program || !programId || !connection || !publicKey) return []
      const capsules = await program.account.timeCapsule.all([
        {
          memcmp: {
            offset: 8,
      ...
      return capsules.map(capsule => ({
        ...
        account: {
          ...
          unlockTime: capsule.account.unlockTime.toNumber(),
        },
      }))
    },
  ...
  const createTimevaultAccount = useMutation<string, Error, CreateVaultArgs>({
    ...
    mutationFn: async ({ unlock_time, title, content_uri, content_type }) => {
      if (!publicKey || !program) throw new Error("Wallet not connected or program not loaded")
      const [capsule] = PublicKey.findProgramAddressSync(
        [Buffer.from("capsule"), publicKey.toBuffer(), new BN(unlock_time).toArrayLike(Buffer, 'le', 8)],
        program.programId
      )
...
export function useTimevaultProgramAccount({ account }: { account: PublicKey }) {
  ...
  const openTimevault = useMutation<string, Error>({
    mutationKey: ['openTimevault', 'open', { cluster, account }],
    mutationFn: async () => {
      if (!program) throw new Error("Program not initialized")
      return program.methods.openCapsule().accounts({ capsule: account }).rpc()
    },
    onSuccess: () => accounts.refetch(),
    onError: (error) => console.error("Failed to open time vault:", error),
  })
...
//   const createTimevaultAccount = useMutation<string, Error, CreateVaultArgs>({
//     mutationKey: ['timevault', 'create', { cluster }],
//     mutationFn: ({ unlock_time, title, content_uri, content_type, userPubkey }) => {
...
//       const capsule = PublicKey.findProgramAddressSync(
//         [Buffer.from("capsule"), userPubkey.toBuffer(), new BN(unlock_time).toArrayLike(Buffer, 'le', 8)],
//         program.programId
//       );
